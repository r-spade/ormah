...
from ormah.integrations.hosts import kilo
...
def test_setup_preserves_config_and_scopes_mcp(tmp_path, project_scope):
    ...
    assert kilo.status(project)['whisper'] == 'native_extension'
    kilo.disconnect(project)
    data = jc.parse(config.read_text()).value
...
def test_native_plugin_contract_with_real_bridge_and_mock_daemon(tmp_path):
    ...
    project.mkdir()
    kilo.connect(project)
    plugin = project / '.kilo/ormah/plugin.mjs'
...
assert.equal(Ormah.id, 'ormah');
const hooks = await Ormah.server({directory: process.argv[3]});
...
''')
    ...
    assert requests[0]['session_id'].startswith('kilo:')


def test_pure_mode_is_not_silently_overridden(tmp_path, monkeypatch):
    monkeypatch.setenv('KILO_PURE', '1')
    with pytest.raises(ValueError, match='KILO_PURE'):
        kilo.connect(tmp_path)
    assert not (tmp_path / 'kilo.jsonc').exists()
    assert kilo.status(tmp_path)['whisper'] == 'unconfigured'
